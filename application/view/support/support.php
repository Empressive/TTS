...
<table id='suggestion' border='1'>
    ...
    if ($rows > 1) {
        foreach ($suggestions as $suggestion) {
            echo "<tr bgcolor='{$suggestion['status_color']}'><td>{$suggestion['now_date']}</td><td>{$suggestion['staff_name']}</td><td>{$suggestion['subject']}</td><td>{$suggestion['comment']}</td></tr>";
        }
    } else {
        echo "<tr bgcolor='{$suggestions['status_color']}'><td>{$suggestions['now_date']}</td><td>{$suggestions['staff_name']}</td><td>{$suggestions['subject']}</td><td>{$suggestions['comment']}</td></tr>";
    }
    ?>
</table>
